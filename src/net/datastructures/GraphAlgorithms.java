...
public class GraphAlgorithms {
	...
	/**
	 * Returns an ordered list of edges comprising the directed path from u to v. If
	 * v is unreachable from u, or if u equals v, an empty path is returned.
	 *
	 * @param g
	 *            Graph instance
	 * @param u
	 ...
	 * @param forest
	 *            must be a map that resulting from a previous call to DFS started
	 *            at u.
	 */
	 ...
	 *
	 *            As an outcome, this method adds newly discovered vertices
	 *            (including s) to the known set, and adds discovery graph edges to
	 *            the forest.
	 */
	...
	/**
	 * Converts graph g into its transitive closure. This uses the Floyd-Warshall
	 * algorithm.
	 */
	...
	public static <V> PositionalList<Vertex<V>> shortestPathList(Graph<V, Integer> g, Vertex<V> src, Vertex<V> target) {
		//Crea una positionalList
		PositionalList<Vertex<V>> path = new LinkedPositionalList<>();
		//Crea un mapa y le asigna el camino mas corto 
